...
const DRAWS_KEY = 'court-draws';
const SESSION_DRAW_ADMIN_ACTIONS = new Set(['runDraw', 'setDrawSettings', 'getDraws']);
...
function settingsOf(state) { return { winners: SD.winnersOf(state && state.drawSettings) }; }
...
async function buildDrawsView(state, store, opts) {
  ...
  return { winnersPerDraw: settings.winners, sessions: view.sessions, hasMore: view.hasMore, nextBefore: view.nextBefore };
}
...
function doSetDrawSettings(state, body, o) {
  const n = Number(body && body.winners);
  if (!SD.isWinnersCount(n)) return bad('Winners per draw must be a whole number from 1 to ' + SD.MAX_WINNERS + '.');
  const prev = settingsOf(state).winners;
  if (prev === n) return { status: 200, body: { ok: true, drawSettings: settingsOf(state), unchanged: true }, changed: false };
  state.drawSettings = Object.assign({}, settingsOf(state), { winners: n });
  pushAudit(state, { action: 'draw.settings', admin: 'admin', at: o.nowMs,
    target: { type: 'drawSettings', id: 'winners', label: 'Winners per draw' }, prevValue: prev, newValue: n });
