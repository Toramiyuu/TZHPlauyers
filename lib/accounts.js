...
/**
 * { playerId: "+60 12-345 6789" } for every linked account that has a number.
 * The whole accounts array is admin-only and heavy (hashes, tokens, encrypted
 * passwords); the Session and Payments screens only ever want to read out a
 * number, so the ops fetch carries this instead. Admin-gated like the rest of
 * adminGetOps — it must never ride the public poll.
 */
function playerPhoneMap(state) {
  const out = {};
  const accounts = (state && Array.isArray(state.accounts)) ? state.accounts : [];
  for (const a of accounts) {
    if (!a || !a.playerId) continue;
    const display = a.phoneDisplay || a.phone || '';
    if (display) out[a.playerId] = String(display);
  }
  return out;
}
...
function doAccountDrawInfo(state, body) {
  ...
  return { status: 200, body: { ok: true, weekly }, changed: false };
}
...
function doAdminSetPhone(state, body) {
  const account = findById(state.accounts, body.id);
  if (!account) return { status: 404, body: { error: 'Account not found.' }, changed: false };
  // An empty number is a deliberate "back to code sign-in only", not a typo: the
  // login code still works, phone + password no longer does. Everything else
  // about the account (password, player link, points) is untouched.
  if (body.phone == null || String(body.phone).trim() === '') {
    const had = account.phone || '';
    if (!had) return { status: 200, body: { ok: true, account: adminAccount(account, state.roster), unchanged: true }, changed: false };
    account.phone = ''; account.phoneDisplay = ''; account.updatedAt = Date.now();
    audit(state, 'account.set_phone', account, { prevValue: had, newValue: '' });
    return { status: 200, body: { ok: true, account: adminAccount(account, state.roster), cleared: true }, changed: true };
  }
  const ph = validatePhone(body.phone);
  if (!ph.ok) return { status: 400, body: { error: ph.error }, changed: false };
  ...
  if (prev === ph.canonical && account.phoneDisplay === ph.display) {
    return { status: 200, body: { ok: true, account: adminAccount(account, state.roster), unchanged: true }, changed: false };
  }
...
module.exports = {
  ...
  findById, findByToken, findByPhone, findByPlayerId, findByCodeKey, playerPhoneMap,
