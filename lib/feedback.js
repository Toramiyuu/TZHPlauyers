 ...
 *   - The client MAY name a night, but only one the server already listed for them
 *     (attendedNights): a night they were actually there for, already started, and inside
 *     the retention window. Anything else is refused.
 ...
 *     and only ever for their LATEST night, so back-filling old ones cannot farm points.
 */
...
// How far back the "pick another night" row goes. Four weeks of Mon/Fri/Sun is plenty of
// catching up; the records themselves live for FB.RETENTION_DAYS, and nothing past the
// prune cutoff is ever offered, so the picker can never point at a night that is about to
// vanish from under it.
const MAX_PICKABLE_NIGHTS = 12;
...
/**
 * The night that owns this instant — the newest night anybody could be writing about.
 * Nothing after it is offered, so a session scheduled ahead is never reviewable.
 */
...
/**
 * Every night this player may write about, newest first.
 *
 * The card is no longer a one-night window that slams shut when the next session starts:
 * a member always gets their latest night expanded and can reach back through the others.
 * Attendance is what admits them — being there, NOT having paid for it. The two ways in:
 *   1. they are in the live line-up for state.sessionDate (attendance is often not ticked
 *      until the next day, and a player walking off court should not hit a wall);
 *   2. they are marked `present` in an attendance record — the same test the session draw
 *      uses, and the only one: `paid` is never consulted.
 *
 * Every date is folded through Night.owningNight, so a stray off-day record (the ghost
 * Saturday) lands on the game night that was actually running. Bounded at both ends: never
 * past the current night, never older than the prune cutoff, never longer than
 * MAX_PICKABLE_NIGHTS. Pure given `nowMs`.
 */
function attendedNights(state, playerId, nowMs) {
  if (!state || !playerId) return [];
  // Default the instant HERE: Night.currentNight reads a missing one as the epoch, which
  // would put every real night in the future and hand back an empty list.
  const now = Number(nowMs) || Date.now();
  const current = openNight(state, now);
  if (!current) return [];
  const today = Night.localDate(now, offsetHours());
  const cutoff = WD.addDaysISO(today, -FB.RETENTION_DAYS);
  const nights = new Set();
  const add = (date) => {
    const night = Night.owningNight(date);
    if (!night || night > current || night < cutoff) return;
    nights.add(night);
  };

  const live = Array.isArray(state.players) ? state.players : [];
  if (live.some((p) => p && p.id === playerId)) add(state.sessionDate);

  const att = isPlainMap(state.attendance) ? state.attendance : {};
  for (const date of Object.keys(att)) {
    ...
    if (entry && entry.present) add(date);
  }

  return Array.from(nights).sort((a, b) => b.localeCompare(a)).slice(0, MAX_PICKABLE_NIGHTS);
}

/** May this player write about this night? The list above is the only authority. Pure. */
function playedThatNight(state, night, playerId, nowMs) {
  if (!night || !playerId) return false;
  return attendedNights(state, playerId, nowMs).includes(night);
}
...
/** One player's own record for a night, trimmed for the wire, or null. Pure. */
function myRecordView(state, night, playerId) {
  const rec = FB.recordFor(state, night, playerId);
  if (!FB.hasContent(rec)) return null;
  return {
 ...
 *
 * `nights` is the whole list, newest first — the first entry is the one the card opens
 * expanded, the rest fill the collapsed "another night" row. `night`/`mine` describe that
 * first entry and are kept for anything reading a single night.
 *
 * `open` false means the card is not shown at all: feedback is switched off, the account
 * has no player linked, or they have not played a night yet.
 * Pure given `nowMs`.
 */
function feedbackViewFor(state, account, nowMs) {
  ...
  const base = {
    open: false, night: '', points: settings.points,
    goodOptions: FB.GOOD_OPTIONS, badOptions: FB.BAD_OPTIONS, mine: null, nights: [],
  };
  if (!settings.enabled || !playerId) return base;
  const dates = attendedNights(state, playerId, nowMs);
  if (!dates.length) return base;

  const nights = dates.map((date, i) => {
    const mine = myRecordView(state, date, playerId);
    return {
      date,
      latest: i === 0,
      mine,
      // The +N is on offer for the LATEST night only. Older nights stay answerable for as
      // long as the picker reaches them, but back-filling a term's worth of sessions must
      // not hand anybody a windfall into this month's draw.
      awardable: i === 0 && settings.points > 0 && !(mine && mine.awarded),
    };
  });
  return Object.assign(base, { open: true, night: nights[0].date, mine: nights[0].mine, nights });
}

/**
 * POST { action:'submitFeedback', token, night?, good[], bad[], goodNote, badNote }.
 * `night` is optional and must be one of the player's own attended nights; left out, it
 * means their latest.
...
function doSubmitFeedback(state, body, opts) {
  ...
  const mine = attendedNights(state, playerId, now);
  if (!mine.length) {
    return { status: 403, body: { error: 'Feedback is for the players who were there that night.' }, changed: false };
  }
  // The client picks which of THEIR nights, and nothing else: an unlisted date is refused
  // rather than quietly redirected, so a stale picker never writes to the wrong night.
  const asked = typeof (body && body.night) === 'string' ? body.night : '';
  const night = asked ? (mine.includes(asked) ? asked : '') : mine[0];
  if (!night) {
    return { status: 403, body: { error: 'That is not one of the nights you played.' }, changed: false };
  }
  ...
  // edit rewrites the record with awarded still true, so it can never pay again. Only the
  // player's LATEST night pays at all — see feedbackViewFor's `awardable`. An older night
  // is left unlatched rather than latched-at-zero: should it ever become payable again the
  // record is still honest about never having been paid.
  let awarded = 0;
  if (!built.record.awarded && night === mine[0]) {
...
module.exports = {
  FEEDBACK_ADMIN_ACTIONS, MAX_PICKABLE_NIGHTS,
  handleFeedbackAdminAction, handleMemberFeedbackAction,
  feedbackViewFor, pruneFeedback,
  // exposed for tests
  attendedNights, playedThatNight, openNight, ensureFeedback, nameFor,
};
