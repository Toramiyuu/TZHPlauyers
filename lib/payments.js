 ...
 * log, exactly like lib/weekly.js. Records live INSIDE the attendance entries
...
function doGeneratePayments(state, body, now, by) {
  ...
  if (changed) {
    if (!day.payments) day.payments = { tier, generatedAt: now, generatedBy: by };
    day.updatedAt = now;
    day.updatedBy = by;
    pushAudit(state, { action: 'payments.generate', admin: by, at: now,
 ...
 * `opts.by` attributes the write: 'admin' (the End-of-the-day button) by default,
 * 'auto' when the 09:00 cron closed the night nobody pressed — see autoCloseNight
 * in api/state.js. It lands on day.payments.generatedBy and the audit entry, so
...
function handlePaymentAdminAction(state, body, opts) {
  try {
    ...
    const by = (opts && opts.by) === 'auto' ? 'auto' : 'admin';
    switch (body && body.action) {
      case 'generatePayments': return doGeneratePayments(state, body, now, by);
