 ...
 *
 * No automatic trigger ever rewrites a record. The one exception is deliberate
 * and admin-only: `removeDraw` replaces a result with a removed-marker (so the
 * sweep cannot quietly redraw the night), and `runDraw` may then overwrite that
 * marker with a fresh result.
 *
...
const SESSION_DRAW_ADMIN_ACTIONS = new Set(['runDraw', 'removeDraw', 'setDrawSettings', 'setDrawPrizes', 'getDraws']);
...
async function doRunDraw(state, body, o) {
  ...
  // point of removing it. Anything else stands.
  ...
  pushAudit(state, { action: 'draw.run', admin: 'admin', at: o.nowMs,
    ...
    prevValue: replacing ? (existing.prev && existing.prev.winners) || [] : undefined,
