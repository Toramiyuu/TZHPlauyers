...
function settingsOf(state) {
  return { winners: SD.winnersOf(state && state.drawSettings), prize: SD.prizeOf(state && state.drawSettings) };
}
...
async function buildDrawsView(state, store, opts) {
  ...
  return { winnersPerDraw: settings.winners, sessionPrize: settings.prize, sessions: view.sessions, hasMore: view.hasMore, nextBefore: view.nextBefore };
}
...
function doSetDrawSettings(state, body, o) {
  ...
  state.drawSettings = { winners, prize };
