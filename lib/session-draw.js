 ...
 *
 * No automatic trigger ever rewrites a record. The exceptions are deliberate and
 * admin-only: `removeDraw` replaces a result with a removed-marker (so the sweep
 * cannot quietly redraw the night), `runDraw` may then overwrite that marker, and
 * `redraw` replaces a result with a fresh one in a single step.
 *
 * Actions (admin-password gated by api/state.js):
 *   runDraw {date}            — manual fallback; refused before the scheduled time or when drawn
 *   redraw {date}             — draw the night again over the existing result (new seed, today's line-up)
 *   removeDraw {date}         — take a result back; leaves a marker, never redrawn on its own
...
const SESSION_DRAW_ADMIN_ACTIONS = new Set(['runDraw', 'redraw', 'removeDraw', 'setDrawSettings', 'setDrawPrizes', 'getDraws']);
...
function memoryDrawStore(initial) {
  ...
  return {
    ...
    // Overwrites whatever is there. Admin-only paths (removeDraw / a re-run after
    // one) — every automatic trigger goes through putIfAbsent.
    async put(date, rec) { map.set(date, JSON.stringify(rec)); return true; },
    size() { return map.size; },
  };
}

/** Redis-hash store. `client` needs hget/hgetall/hsetnx/hset (the kv façade in api/state.js). */
function redisDrawStore(client, key) {
  ...
  return {
    ...
    async put(date, rec) { await client.hset(k, date, JSON.stringify(rec)); return true; },
  };
}
...
// ── admin actions ────────────────────────────────────────────────────
/** The session an admin draw is about, or the error to answer with. */
function drawTargetFor(state, date, o) {
  const cand = SD.sessionCandidates(state, o.offsetHours).find((c) => c.date === date);
  if (!cand) return { err: { status: 404, body: { error: 'No session found for that date.' }, changed: false } };
  if (!Number.isFinite(cand.drawAt)) return { err: bad('No draw is scheduled for that session.') };
  if (o.nowMs < cand.drawAt) {
    return { err: { status: 400, body: { error: 'Draw time not reached yet — scheduled for ' + SD.fmtDrawTime(cand.drawAt) + '.', drawAt: cand.drawAt }, changed: false } };
  }
  return { cand };
}
/** A fresh result for that session: same rules as the cron, new seed, run by hand. */
function adminDrawResult(state, date, cand, o) {
  return SD.buildDrawResult({
...
/** The winners a stored record (result or removed-marker) is holding, by name. */
function winnerNamesOf(rec) {
  if (!rec) return [];
  if (SD.isRemovedRecord(rec)) return (rec.prev && rec.prev.winners) || [];
  return (rec.winners || []).map((id) => (rec.names && rec.names[id]) || id);
}
...
async function doRunDraw(state, body, o) {
  ...
  // A removed result is the one thing a re-run may overwrite — that is the whole
  // point of removing it. Anything else stands (see doRedraw for replacing one).
  const replacing = SD.isRemovedRecord(existing);
  if (existing && !replacing) return { status: 409, body: { error: 'This session has already been drawn.', result: existing }, changed: false };
  const target = drawTargetFor(state, date, o);
  if (target.err) return target.err;
  const rec = adminDrawResult(state, date, target.cand, o);
  const wrote = replacing && typeof o.store.put === 'function'
    ? await o.store.put(date, rec)
    : await o.store.putIfAbsent(date, rec);
  if (!wrote) {
  ...
  pushAudit(state, { action: 'draw.run', admin: 'admin', at: o.nowMs,
    ...
    prevValue: replacing ? winnerNamesOf(existing) : undefined,
    newValue: winnerNamesOf(rec),
    note: 'manual · ' + rec.counts.eligible + ' eligible' + (rec.shortfall ? ' · short of ' + rec.winnersWanted : '')
      + (replacing ? ' · replaces a removed result' : '') });
  return { status: 200, body: { ok: true, result: rec, replaced: replacing }, changed: true };
}

/**
 * Draw the night again in one step: the remove-and-run-again pair an admin would
 * otherwise do by hand, without the window where the night has no result at all.
 * A new seed over the line-up as it stands NOW, so fixing the payments first is
 * what makes the second draw different. The old winners live on in the audit log.
 */
async function doRedraw(state, body, o) {
  const date = body && body.date;
  if (!SD.isValidISO(date)) return bad('Invalid date.');
  if (!SD.isDrawDay(date)) return bad('No draw is scheduled for that day of the week.');
  if (typeof o.store.put !== 'function') return { status: 500, body: { error: 'Draw storage cannot replace a result.' }, changed: false };
  const existing = await o.store.get(date);
  if (!existing) return { status: 404, body: { error: 'There is no draw result for that session. Use "Run draw now" instead.' }, changed: false };
  const target = drawTargetFor(state, date, o);
  if (target.err) return target.err;
  const prevWinners = winnerNamesOf(existing);
  const rec = adminDrawResult(state, date, target.cand, o);
  await o.store.put(date, rec);
  pushAudit(state, { action: 'draw.redraw', admin: 'admin', at: o.nowMs,
    target: { type: 'sessionDraw', id: date, label: 'Session draw ' + date },
    prevValue: prevWinners, newValue: winnerNamesOf(rec),
    note: 'drawn again · ' + rec.counts.eligible + ' eligible' + (rec.shortfall ? ' · short of ' + rec.winnersWanted : '') });
  return { status: 200, body: { ok: true, result: rec, prevWinners }, changed: true };
}

/**
 * Take a result back. The record is replaced with a removed-marker rather than
 * deleted: an empty field would be redrawn by the very next sweep, which is the
 * opposite of what the admin asked for. The night reads as pending again and
 * only "Run draw now" can put a new result there.
 */
async function doRemoveDraw(state, body, o) {
  const date = body && body.date;
  if (!SD.isValidISO(date)) return bad('Invalid date.');
  if (typeof o.store.put !== 'function') return { status: 500, body: { error: 'Draw storage cannot remove a result.' }, changed: false };
  const existing = await o.store.get(date);
  if (!existing) return { status: 404, body: { error: 'There is no draw result for that session.' }, changed: false };
  if (SD.isRemovedRecord(existing)) {
    return { status: 400, body: { error: 'That result has already been removed.', alreadyRemoved: true }, changed: false };
  }
  const marker = SD.removedDrawRecord(existing, o.nowMs);
  await o.store.put(date, marker);
  pushAudit(state, { action: 'draw.remove', admin: 'admin', at: o.nowMs,
    target: { type: 'sessionDraw', id: date, label: 'Session draw ' + date },
    prevValue: marker.prev.winners, newValue: [],
    note: 'result removed · was ' + (marker.prev.method === 'manual' ? 'run by an admin' : 'drawn automatically') });
  return { status: 200, body: { ok: true, date, removed: marker }, changed: true };
}
...
async function handleSessionDrawAdminAction(state, body, opts) {
  try {
    ...
    switch (body && body.action) {
      ...
      case 'redraw':          return await doRedraw(state, body, ctx);
      case 'removeDraw':      return await doRemoveDraw(state, body, ctx);
