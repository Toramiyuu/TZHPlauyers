...
// ── Shuttlecock auto-enrolment was removed with the draw (2026-09): attendance
//    records survive it untouched, but the two eligibility actions are gone. ──
let s5 = freshState();
...
check('attendance still recorded for every night', ['2026-07-06', '2026-07-13', '2026-07-20', '2026-07-27']
  .every(d => s5.attendance[d] && s5.attendance[d].entries.p6 && s5.attendance[d].entries.p6.paid));
check('retired recomputeMonthly action is rejected',
  W.handleWeeklyAdminAction(s5, { action: 'recomputeMonthly', month: '2026-07' }).status === 400 && !W.WEEKLY_ADMIN_ACTIONS.has('recomputeMonthly'));
check('retired setMonthlyOverride action is rejected',
  W.handleWeeklyAdminAction(s5, { action: 'setMonthlyOverride', month: '2026-07', playerId: 'p9', eligible: true }).status === 400 && !W.WEEKLY_ADMIN_ACTIONS.has('setMonthlyOverride'));
check('neither an eligibility cache nor a ballot is created any more', s5.monthlyEligibility === undefined && s5.monthlyDraw === undefined);
