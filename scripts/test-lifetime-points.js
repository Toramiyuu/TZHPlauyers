...
// ── client ──
// Pinned on the one key that matters rather than the whole initializer, so adding an
// unrelated field to the ops cache doesn't fail a lifetime-points test.
check('the ops cache starts with lifetimePoints null — "not fetched", not zero',
  /^let adminOps = \{.*\blifetimePoints: null\b.*\};$/m.test(html));
